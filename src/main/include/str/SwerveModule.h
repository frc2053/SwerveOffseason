...
public:
  ...
  // We return the optimized state to make sure we log what we actually set the
  // module to
  frc::SwerveModuleState GoToState(frc::SwerveModuleState desiredState,
                                   bool optimize = true, bool openLoop = false,
                                   units::ampere_t arbFF = 0_A);
...
private:
  ...
  bool ConfigureDriveMotor(bool invertDrive, units::ampere_t supplyCurrentLimit,
